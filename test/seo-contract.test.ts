...
function htmlPages(): string[] {
  return Object.keys(DOCUMENTATION).filter((path) => !path.endsWith(".md") && path !== "/login");
}

function jsonLd(html: string): Array<Record<string, unknown>> {
  const script = html.match(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/u)?.[1];
...
it("publishes the SEO contract for every rendered page", async () => {
  ...
  expect([...Object.keys(PAGE_META)].sort()).toEqual([...paths, "/login"].sort());
  for (const [path, meta] of Object.entries(PAGE_META))
    if (meta.parent) expect(Object.keys(PAGE_META), `${path} parent`).toContain(meta.parent);
  ...
  for (const path of paths) {
    ...
    expect(html.match(/<meta name="description"/gu), `${path} description count`).toHaveLength(1);
    const description = attribute(html, 'name="description"');
    expect(description.length, `${path} description length`).toBeGreaterThanOrEqual(120);
    ...
    expect(html).toContain(`<link rel="canonical" href="https://ohmyho.st${path}">`);
    expect(html).toContain(`<link rel="alternate" type="text/markdown" href="${path}.md">`);
    ...
    expect(crumbs.itemListElement.at(-1)?.item).toBe(`https://ohmyho.st${path}`);
    const footer = html.slice(html.lastIndexOf("<footer>"));
    expect(footer, `${path} footer`).toContain(footerColumnsHtml());
    expect(footer.match(/href="\//gu)?.length ?? 0, `${path} footer links`).toBeGreaterThanOrEqual(
      15,
    );
    ...
    for (const [anchor] of main.matchAll(/<a href="https?:\/\/(?!ohmyho\.st\/)[^>]*>/gu))
    ...
    if (/^## FAQ$/mu.test(DOCUMENTATION[path] ?? "")) {
      ...
      const section = (DOCUMENTATION[path] ?? "").split(/^## FAQ$/mu)[1]?.split(/^## /mu)[0] ?? "";
      expect(faq.mainEntity, `${path} FAQ`).toHaveLength(section.match(/^### /gmu)?.length ?? 0);
    }
    if (meta.kind === "howto") {
      const howTo = graph.find((node) => node["@type"] === "HowTo") as { step: unknown[] };
      expect(howTo.step.length, `${path} HowTo steps`).toBeGreaterThanOrEqual(3);
    }
    if (meta.kind === "article") {
      const post = graph.find((node) => node["@type"] === "BlogPosting") as Record<string, unknown>;
  ...
  expect(blog.blogPost).toHaveLength(paths.filter((path) => path.startsWith("/blog/")).length);
...
it("serves crawler, cache and transport metadata", async () => {
  ...
  const robots = await worker.fetch(new Request("https://ohmyho.st/robots.txt"));
  const rules = await robots.text();
  expect(rules).toContain("User-agent: *\nAllow: /\nDisallow: /login\nDisallow: /*?r=\n");
  for (const agent of ["GPTBot", "ClaudeBot", "anthropic-ai", "PerplexityBot", "OAI-SearchBot"])
  ...
  for (const path of ["/", "/brand", "/philosophy", "/open-source", "/blog", ...htmlPages()])
    expect(listed, path).toContain(path);
  expect(listed).not.toContain("/login");
  expect(listed.some((path) => path.endsWith(".md"))).toBe(false);
  for (const [path, meta] of Object.entries(PAGE_META))
    if (path !== "/login")
      expect(urls.find((entry) => entry[1] === path)?.[2], path).toBe(meta.modified);
  for (const url of [
    "https://ohmyho.st/",
  ...
  expect(
    (await worker.fetch(new Request("https://ohmyho.st/about"))).headers.get("cache-control"),
  ).toBe("no-store");
  const font = await worker.fetch(new Request("https://ohmyho.st/fonts/3386a05f6ece969e.ttf"), {
    ASSETS: assets,
  });
  expect(font.headers.get("cache-control")).toBe("public, max-age=31536000, immutable");
  const logo = await worker.fetch(new Request("https://ohmyho.st/logos/workos.svg"), {
    ASSETS: assets,
  });
  expect(logo.headers.get("cache-control")).toBe("public, max-age=86400");
  const favicon = await worker.fetch(new Request("https://ohmyho.st/favicon.svg"));
...
it("hardens the approved homepage markup for search and social", async () => {
  ...
  expect(brand).toContain('<meta property="og:image" content="https://ohmyho.st/og.png">');
});
