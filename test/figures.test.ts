...
import { credits, number, priceLine, priceWorkload, scenarioUsd } from "../src/content/format.js";
import { DATABASE_PROFILES, SCENARIOS, WORKLOADS } from "../src/content/sources.js";
import {
  billsScenario,
  comparisonColumns,
  creditBarRows,
...
function values(html: string): string[] {
  return [...html.matchAll(/data-value="([^"]+)"/gu)].map((match) => match[1] ?? "");
}
...
it("renders every figure as one Markdown-safe block whose numbers come from the data module", () => {
  for (const html of [
    ...
    expect(html).not.toMatch(/<script|<image|<foreignObject|href="http[^"]*"[^>]*>(?![^<]*<\/a>)/u);
    const rendered = marked.parse(`${html}\n`, { async: false });
    expect(rendered.trim()).toBe(html);
    expect(rendered).not.toContain("<p>");
  }
  for (const competitor of ["vercel", "supabase", "resend", "railway"] as const) {
    const scenario = billsScenario(competitor);
    const expected = [
      ...scenario.parts.map((part) => String(part.fact.usd * (part.times ?? 1))),
      String(scenarioUsd(scenario)),
      "10",
    ];
    expect(values(figureBills(competitor))).toEqual(expected);
    expect(figureBills(competitor)).toContain("List prices checked on September 19, 2026");
    for (const vendor of new Set(scenario.parts.map((part) => part.vendor)))
      expect(figureBills(competitor)).toContain(`href="${vendor.sourceUrl}" rel="noopener"`);
  ...
  expect(figureCreditBars()).toContain('data-count-to="552.44" data-suffix=" credits"');
  const catalog = new Set(mcpTools.tools.map((tool: { name: string }) => tool.name));
  for (const tool of flowToolNames()) expect(catalog.has(tool), tool).toBe(true);
  expect(figureFlow("Codex")).toContain(`${mcpTools.tools.length}-tool catalog`);
...
it("keeps the pinned homepage's bill and credit examples equal to the data module", async () => {
  const home = await readFile(new URL("../public/pages/home.html", import.meta.url), "utf8");
  expect(home).toContain(`data-to="${scenarioUsd(SCENARIOS.threeSubscriptions)}"`);
  expect(home).toContain(
    `Resend Pro $20 — around $${scenarioUsd(SCENARIOS.threeSubscriptions)} for one project`,
  );
  const round = (microcredits: number) => `${Math.round(microcredits / MICROCREDITS)} credits`;
  expect(home).toContain(
