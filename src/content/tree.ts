...
const PAGE_KINDS = new Set<PageKind>(["page", "article", "howto", "about", "collection"]);
const PAGE_STATUS = new Set(["published", "draft"]);

/** One page as it is written in the content tree, before its tokens are resolved. */
export interface ContentSource extends Omit<ContentPage, "ogImage" | "ogImageAlt"> {
  readonly status: "published" | "draft";
  readonly social?: { readonly image?: string | null; readonly alt?: string | null };
}

/** Every page folder under `content/`, as its URL. The folder path is the URL. */
export function pageFolders(base: string): Array<{ url: string; directory: string }> {
  ...
  const walk = (directory: string, url: string): void => {
    for (const entry of readdirSync(directory).sort()) {
      ...
      if (readdirSync(next).includes("page.json")) found.push({ url: nextUrl, directory: next });
      walk(next, nextUrl);
    }
  };
  walk(base, "");
...
export function readPage(directory: string, url: string): ContentSource {
  const meta = JSON.parse(readFileSync(join(directory, "page.json"), "utf8")) as Record<
    string,
    unknown
  >;
  ...
  if (!PAGE_KINDS.has(kind)) fail(`kind must be one of ${[...PAGE_KINDS].join(", ")}`);
  if (!PAGE_STATUS.has(meta["status"] as string)) fail("status must be published or draft");
  if (title.length > 60) fail(`title is ${title.length} characters, at most 60`);
  ...
  return {
    ...
    ...(meta["social"] ? { social: meta["social"] as ContentSource["social"] } : {}),
...
export function resolveTokens(
  markdown: string,
  path: string,
  render: (kind: string, args: string[], options: Record<string, string>) => string,
): string {
  return markdown.replaceAll(/\{\{\s*(.+?)\s*\}\}/gsu, (whole, body: string) => {
