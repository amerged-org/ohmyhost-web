...
import { PLANS, SCENARIOS, VENDORS } from "../content/sources.js";

export const page: ContentPage = {
  path: "/about",
  title: `Who runs ohmyho.st: Amerged B.V. in Venray, Netherlands`,
  description: `ohmyho.st is run by Amerged B.V. (KVK 42154221, Venray, Netherlands), founded by Sebastian Mertens: hosting for vibe-coded apps your agent deploys from GitHub.`,
  kind: "about",
  modified: "2026-09-20",
  crumb: "About",
  markdown: `# Who runs ohmyho.st

ohmyho.st is run by Amerged B.V., a Dutch company registered at the KVK under number 42154221 in Venray, Limburg, and founded by Sebastian Mertens. It hosts vibe-coded apps that your coding agent deploys from GitHub. Free gives ${number(PLANS.freeCredits)} credits a month; Paid is ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits shared by every project.
...
The founder ran several small apps and paid for each one three times: hosting, a database and a mail sender, each on its own subscription with its own dashboard. One developer on Vercel Pro, Supabase Pro and Resend Pro pays ${usd(scenarioUsd(SCENARIOS.threeSubscriptions))} a month for one project before any usage, and Supabase adds ${usd(VENDORS.supabase.facts.microProject.usd)} a month for each additional project's Micro instance. Most of those projects were idle most of the month.

ohmyho.st is the answer to that bill. It buys hosting, Postgres, domains and transactional mail from the providers underneath, adds a small margin and meters what each project actually uses from one prepaid balance. Free gives ${number(PLANS.freeCredits)} credits per UTC month. Paid is ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits per period. Monthly credits expire at the end of the period; purchased top-ups never expire, at ${number(PLANS.topUpPerUsd)} credits per dollar. Every project draws from the organization's one balance, and there is no per-project base fee. A quiet project pays for what it keeps: retained database storage at ${rate("neon.storage.root")} and a deployed script at about ${credits(priceLine("wfp.script", 1))} a month. A zero balance starts a ${number(PLANS.graceDays)}-day grace period; funded services keep running, and only unfunded services suspend afterwards. The reasoning is on the [philosophy page](/philosophy).
...
Nothing at first. Free gives ${number(PLANS.freeCredits)} credits per UTC month, and they expire at the end of the month. Paid is ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits per period, and top-ups at ${number(PLANS.topUpPerUsd)} credits per dollar never expire. Every project draws from the same balance; there is no per-project base fee.
