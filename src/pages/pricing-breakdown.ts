...
import { figureCreditBars } from "../figures.js";
import { CREDIT_PRICING_TABLE } from "../generated-pricing.js";

export const page: ContentPage = {
  path: "/pricing/breakdown",
  title: `Cheaper than Vercel? The ohmyho.st cost breakdown`,
  description: `Every published credit rate, four worked workloads and the three subscriptions they replace, plus where ohmyho.st costs more than Vercel, Resend or Railway.`,
  kind: "page",
  modified: "2026-09-20",
  parent: "/pricing",
  crumb: "Cost breakdown",
  markdown: `# Cheaper than Vercel? The ohmyho.st cost breakdown

Yes for several small projects, no for one busy app. ${usd(PLANS.paidUsd)} a month buys ${number(PLANS.paidCredits)} credits shared by every project; a small app uses about ${credits(priceWorkload(WORKLOADS.smallApp).microcredits)} of them, a quiet side project about ${credits(priceWorkload(WORKLOADS.quietProject).microcredits)}. Vercel Pro, Supabase Pro and Resend Pro bought separately cost ${usd(scenarioUsd(SCENARIOS.threeSubscriptions))} a month before any usage.

## What a credit is

One credit has a nominal value of ${usd(PLANS.usdPerCredit)}, so ${number(PLANS.topUpPerUsd)} credits represent ${usd(1)} of credit value. Every rate on this page comes from one formula: the provider's list cost in dollars times 1.15, divided by 0.0035. That is the whole margin. Each rate rounds up to the next microcredit (one credit is a million microcredits), no meter has a minimum charge, and a balance never goes negative. Provider free tiers are not passed through: you pay the list-cost rate from the first unit, which is also why nothing on this page depends on a quota you might exceed.

Credits arrive three ways:

- Free grants ${number(PLANS.freeCredits)} credits per UTC month. They expire at the end of that month.
- Paid costs ${usd(PLANS.paidUsd)} a month before tax and grants ${number(PLANS.paidCredits)} credits per paid period. They expire at the end of the period, with no rollover. Paid also unlocks a customer-owned domain and verified sender mail; both still use credits.
- Top-ups never expire. A purchase grants ${number(PLANS.topUpPerUsd)} credits per dollar up to ${usd(100)} and ${number(PLANS.topUpPerUsdAbove100)} credits per dollar for the part above that. Monthly credits are spent before top-up credits, so the ones that expire go first.
...
How to read it: the Credits column is what you pay for the measured quantity, and a charge is that rate times the measured amount, rounded up. Requests and CPU are two separate meters, so a request that burns CPU costs more than one that returns quickly. Functions and crons have no meter of their own; each run is one request plus its CPU-ms. The two compute rows are database profiles. Free runs at ${number(DATABASE_PROFILES.free.cu)} CU and Paid standard at ${number(DATABASE_PROFILES.standard.cu)} CU, so one active hour on Paid standard is half a CU-hour, about ${credits(priceLine("neon.compute.scale", DATABASE_PROFILES.standard.cu))}. The performance profile is ${number(DATABASE_PROFILES.performance.cu)} CU on its own row, about ${credits(priceLine("neon.compute.performance", DATABASE_PROFILES.performance.cu))} per active hour, which is 2.5 times Paid standard for the same active minute. The r2 rows meter uploaded files: storage by daily peak, then writes and reads by operation. Sender mail is charged at the Essentials row, per recipient; To, CC and BCC each count. The a-la-carte row is a published card, not the row a verified sender uses.

The build row deserves a closer look. It reads ${rate("build.sandbox.standard-3")}, but the code does not store a per-second decimal. It stores the exact sandbox rational, ${number(priceLine("build.sandbox.standard-3", 175) / 1_000_000, 4)} for every ${number(175)} build seconds, and prices each build from its measured seconds, rounded up to the full second. Sandbox start-up time is not billed. A deployment plan holds the build price before the build starts and releases what the build did not use. So 20 build minutes cost ${credits(priceLine("build.sandbox.standard-3", 1200), 6)}, and the workloads below all use that figure.

## Where the credits go

${figureCreditBars()}

The bars put the rate card in proportion. A million requests cost about ${credits(priceLine("wfp.requests", 1_000_000))}; one active database hour on Paid standard about ${credits(priceLine("neon.compute.scale", DATABASE_PROFILES.standard.cu))}; one stored gigabyte for a month ${rate("neon.storage.root")}. For a side project the database, not the traffic, is where the credits go: eight active hours cost more than a hundred thousand requests. The two Paid-only fixed lines, a custom hostname at about ${credits(priceLine("domain.custom_hostname", 1))} a month and a mail sender zone at about ${credits(priceLine("route53.zone", 1))} a month, are what turn a quiet project into a client site. The last bar is the small app, itemized in the next section.
...
${workloadTable(WORKLOADS.clientSite)}
...
Two hundred active database hours and ten stored gigabytes dominate; traffic and mail come next. This is the workload where the ${number(PLANS.paidCredits)} monthly credits are a small fraction of the total and the rest is bought as top-ups, which is the honest way to say ohmyho.st is not the cheapest place for it.

## The three subscriptions, bought separately

The classic side-project stack is three accounts:

- Vercel Pro: ${usd(VENDORS.vercel.facts.pro.usd)} a month for one developer seat.
- Supabase Pro: from ${usd(VENDORS.supabase.facts.pro.usd)} a month, which covers the first project's Micro instance. Each additional project's Micro instance is ${usd(VENDORS.supabase.facts.microProject.usd)} a month.
- Resend Pro: ${usd(VENDORS.resend.facts.pro.usd)} a month.

For one developer and one project that is ${usd(scenarioUsd(SCENARIOS.threeSubscriptions))} a month before usage above the plans and before tax. Vercel Pro and Resend Pro are per account, so five projects do not multiply them; Supabase is per project, so the same stack for five projects is ${usd(scenarioUsd(SCENARIOS.fiveProjects))} a month.

On ohmyho.st the same five projects share one ${usd(PLANS.paidUsd)} balance. Five quiet projects together use about ${credits(priceWorkload(WORKLOADS.fiveQuietProjects).microcredits)}, inside the ${number(PLANS.paidCredits)} monthly credits with room left; the small app alone uses about ${credits(priceWorkload(WORKLOADS.smallApp).microcredits)}. Two caveats. The free tiers on all three services narrow the gap for a single personal project, and the three plans buy capacity and features the credit tables do not replicate, so read the workload tables against your own numbers rather than the headline.
...
- Each deployed script, about ${credits(priceLine("wfp.script", 1))} a month. Every deployment stages one immutable script that stays for rollback until it is cleaned up or the project is deleted.
- A linked custom hostname, about ${credits(priceLine("domain.custom_hostname", 1))} a month, Paid.
- A mail sender zone, about ${credits(priceLine("route53.zone", 1))} a month, Paid. It is free if removed within twelve hours; otherwise it costs a full month, then a month on each first of the month while it exists.
...
The busy app. One busy production app uses about ${credits(priceWorkload(WORKLOADS.busyApp).microcredits)} a month, ${creditValueUsd(priceWorkload(WORKLOADS.busyApp).microcredits)} of credit value, most of it in database compute and storage. At that scale Vercel Pro at ${usd(VENDORS.vercel.facts.pro.usd)} a month with its usage allowance and Supabase Pro from ${usd(VENDORS.supabase.facts.pro.usd)} a month with a fixed compute instance can come out cheaper, and a usage meter stops being an advantage. Compare a concrete quote before moving a busy app.

Resend at high volume. Resend Pro is ${usd(VENDORS.resend.facts.pro.usd)} a month for 50,000 emails. The same 50,000 recipients on ohmyho.st cost about ${credits(priceWorkload(WORKLOADS.fiftyThousandRecipients).microcredits)}, ${creditValueUsd(priceWorkload(WORKLOADS.fiftyThousandRecipients).microcredits)} of credit value, and Resend Scale is ${usd(VENDORS.resend.facts.scale.usd)} a month for 100,000. Below a few thousand recipients the per-recipient meter is cheaper because there is no plan to buy; in the tens of thousands, Resend is.

Railway for containers. Railway runs any container and bills CPU and memory by the second above a ${usd(VENDORS.railway.facts.pro.usd)} a month minimum on Pro. ohmyho.st runs no containers. It deploys Next.js, Vite/React and TanStack Start from a GitHub repository the customer authorizes, and nothing else. A long-running process, a websocket server you manage yourself or a binary dependency belongs on Railway, whatever the credits say.
...
For several small projects, yes: ${usd(PLANS.paidUsd)} a month buys ${number(PLANS.paidCredits)} credits shared by every project, and five quiet projects use about ${credits(priceWorkload(WORKLOADS.fiveQuietProjects).microcredits)}. Vercel Pro alone is ${usd(VENDORS.vercel.facts.pro.usd)} a month. For one busy production app with heavy database use the usage total can exceed the subscriptions, so price your own workload first.

### Do unused credits roll over?

Monthly credits do not: the ${number(PLANS.freeCredits)} Free credits expire at the end of the UTC month and the ${number(PLANS.paidCredits)} Paid credits at the end of the paid period. Top-up credits never expire and are spent after the monthly ones, so a top-up is the way to bank credits for a busy month later.

### What does a project cost when nobody visits it?

Only what it keeps. Stored data at ${rate("neon.storage.root")}, one deployed script at about ${credits(priceLine("wfp.script", 1))} a month, and on Paid a custom hostname or mail sender zone if you linked one. Database compute suspends after a minute of idle time and costs nothing until the next query wakes it.
...
Because the sandbox is billed per second and the rate is kept exact. The published row is ${rate("build.sandbox.standard-3")}; in code it is stored as ${number(priceLine("build.sandbox.standard-3", 175) / 1_000_000, 4)} for every ${number(175)} seconds so no per-second decimal ever rounds. Twenty build minutes therefore cost ${credits(priceLine("build.sandbox.standard-3", 1200), 6)}, the figure every table on this page uses.

${sourcesSection(VENDORS.vercel, VENDORS.supabase, VENDORS.resend, VENDORS.railway)}
