...
export const page: ContentPage = {
  path: "/for/codex",
  title: `Codex deploy: hosting for vibe-coded apps — ohmyho.st`,
  description: `Deploy from Codex with one prompt: 62 MCP tools, plan-then-execute deploys, a non-expiring token and a stop budget so an agent can run unattended.`,
  kind: "howto",
  modified: "2026-09-20",
  crumb: "Codex",
  markdown: `# Deploy with Codex

Paste one prompt into Codex and it deploys your GitHub repository to ohmyho.st through 62 MCP tools: hosting, Postgres, a domain and mail from one balance. Free gives ${number(PLANS.freeCredits)} credits a month; Paid is ${usd(PLANS.paidUsd)} for ${number(PLANS.paidCredits)}. A non-expiring token and a stop budget let the agent run without you watching.
...
2. \`project_budget_set\` as the ceiling. A budget is a UTC-month limit in microcredits (${number(1_000_000)} microcredits is one credit) with two modes. \`continue\` keeps drawing from the organization's balance past the threshold; \`stop\` rejects new billable work with \`project_budget_exceeded\` and pauses the project's traffic with HTTP 402. Set stop at the Free allowance, ${number(PLANS.freeCredits)} credits, and a loop that redeploys every failed build ends on its own instead of draining your top-ups. Changing a budget never resets usage or adds credits ([budgets](https://docs.ohmyho.st/budgets)).
...
Free: ${number(PLANS.freeCredits)} credits per UTC month. Paid: ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits per period. Monthly credits expire at period end; purchased top-ups never expire, at ${number(PLANS.topUpPerUsd)} credits per dollar up to ${usd(100)} and ${number(PLANS.topUpPerUsdAbove100)} per dollar above. Every project draws from the organization's one balance; there is no per-project base fee.

An agent loop spends on builds, requests, database time and whatever it keeps. One active hour on the Paid standard profile (${DATABASE_PROFILES.standard.cu} CU) is about ${credits(priceLine("neon.compute.scale", DATABASE_PROFILES.standard.cu))}. Idle compute suspends, but stored data stays at ${rate("neon.storage.root")} and each deployed script at about ${credits(priceLine("wfp.script", 1))} a month. Workers bandwidth is not charged.

${workloadTable(WORKLOADS.agentLoop)}

Compare that total with ${number(PLANS.freeCredits)} Free or ${number(PLANS.paidCredits)} Paid credits a month, and set the stop budget from it. The full rate card is at [/pricing/breakdown](/pricing/breakdown) and [docs.ohmyho.st/pricing](https://docs.ohmyho.st/pricing).
