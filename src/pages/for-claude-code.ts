...
export const page: ContentPage = {
  ...
  description: `Paste one prompt into Claude Code: it connects to ohmyho.st over MCP, links your GitHub repo, plans the deployment, sets secrets and verifies the live app.`,
  kind: "howto",
  modified: "2026-09-20",
  crumb: "Claude Code",
  markdown: `# Deploy a Claude Code app in one prompt

Paste one prompt into Claude Code and it deploys your GitHub repository to ohmyho.st over MCP: it links the repo, plans the build, sets secrets and returns a live URL. Free is ${number(PLANS.freeCredits)} credits a month; Paid is ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits shared by every project. A small app for one month uses about ${credits(priceWorkload(WORKLOADS.smallApp).microcredits)}.
...
A small app for one month uses about ${credits(priceWorkload(WORKLOADS.smallApp).microcredits)}, about ${creditValueUsd(priceWorkload(WORKLOADS.smallApp).microcredits)} of credit value, and the table shows where each credit goes.

${workloadTable(WORKLOADS.smallApp)}

Free gives ${number(PLANS.freeCredits)} credits per UTC month. Paid is ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits per period. Monthly credits expire at period end; purchased top-ups never expire and grant ${number(PLANS.topUpPerUsd)} credits per dollar up to ${usd(100)} and ${number(PLANS.topUpPerUsdAbove100)} per dollar for the part above. Every project draws from the organization's one balance; there is no per-project base fee. A zero balance starts a ${number(PLANS.graceDays)}-day grace period in which funded services keep running; afterwards only unfunded services suspend. Automatic recharge is not enabled in the current beta.

A quiet month is cheaper. Idle database compute suspends; what remains is retained storage at ${rate("neon.storage.root")}, the deployed script at about ${credits(priceLine("wfp.script", 1))} a month, a linked custom hostname at about ${credits(priceLine("domain.custom_hostname", 1))} a month (Paid) and a mail sender zone at about ${credits(priceLine("route53.zone", 1))} a month (Paid). ${WORKLOADS.quietProject.name} comes to about ${credits(priceWorkload(WORKLOADS.quietProject).microcredits)}. Requests are ${rate("wfp.requests")}; Workers bandwidth is not charged and SQL exports are free. One active hour on the Paid standard ${DATABASE_PROFILES.standard.cu} CU database is about ${credits(priceLine("neon.compute.scale", DATABASE_PROFILES.standard.cu))}.
...
${WORKLOADS.quietProject.name} comes to about ${credits(priceWorkload(WORKLOADS.quietProject).microcredits)}: two build minutes, a few thousand requests, one active database hour, a fifth of a gigabyte of storage and the deployed script. Idle database compute suspends; storage at ${rate("neon.storage.root")} and the script at about ${credits(priceLine("wfp.script", 1))} a month keep using credits. Workers bandwidth is not charged.
