...
export const page: ContentPage = {
  path: "/philosophy",
  title: `Hosting not per project: why one balance — ohmyho.st`,
  description: `Why ohmyho.st meters every project from one balance instead of charging per project: buy in bulk, sell at cost plus a little, export and leave when you want.`,
  kind: "page",
  modified: "2026-09-20",
  crumb: "Philosophy",
  markdown: `# Hosting not per project: why one balance

ohmyho.st prices hosting per organization, not per project. One balance funds every project: Free gives ${number(PLANS.freeCredits)} credits a month, Paid is ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits, and each project draws what it uses. No per-project base fee, no deploy dashboard, and an export that works without asking. Rates are the provider's cost plus a small margin.
...
Hosting is usually priced per project, per seat or per service. That punishes the person with nine side projects and two live ones. Supabase Pro starts at ${usd(VENDORS.supabase.facts.pro.usd)} a month, and each additional project's Micro instance is ${usd(VENDORS.supabase.facts.microProject.usd)} a month. Vercel Pro is ${usd(VENDORS.vercel.facts.pro.usd)} a month, and each developer seat is ${usd(VENDORS.vercel.facts.developerSeat.usd)}. Put five side projects on Vercel Pro, Supabase Pro and Resend Pro and the list price is ${usd(scenarioUsd(SCENARIOS.fiveProjects))} a month before you serve one request.
...
A single busy production app with serious traffic may still belong on Vercel and Supabase. Priced here, one busy production app for one month is about ${credits(priceWorkload(WORKLOADS.busyApp).microcredits)}, about ${creditValueUsd(priceWorkload(WORKLOADS.busyApp).microcredits)} of credit value. Vercel Pro at ${usd(VENDORS.vercel.facts.pro.usd)}, Supabase Pro from ${usd(VENDORS.supabase.facts.pro.usd)} and Resend Pro at ${usd(VENDORS.resend.facts.pro.usd)} list at ${usd(scenarioUsd(SCENARIOS.threeSubscriptions))} together. If that usage fits inside their plans, they are cheaper. Metering wins when you have many small things; a plan wins when you have one big thing that fills it.

Vercel Hobby is ${usd(VENDORS.vercel.facts.hobby.usd)} a month for one developer seat. If that is all your projects need, it beats any paid balance, and I will not pretend otherwise.

Railway, Fly.io and Render run any container. ohmyho.st runs Next.js, Vite/React and TanStack Start on Workers, deployed from a GitHub repository, and nothing else. Resend Pro is ${usd(VENDORS.resend.facts.pro.usd)} for 50,000 emails a month; here mail is ${rate("ses.{region}.recipients (Essentials)")}, counted per recipient, and it needs Paid plus a verified sender subdomain. Send most of that plan every month and Resend is the better price. Supabase gives you Auth, Storage, Realtime and Edge Functions in one console; here you bring Better Auth or WorkOS AuthKit and write plain SQL.

Every comparison page says where the other side wins, with list prices checked on a named date. ${checkedLine(VENDORS.vercel)} ${checkedLine(VENDORS.supabase)} ${checkedLine(VENDORS.resend)}

## Dated prices as a principle

Every comparison page on this site carries the date its list prices were checked and links the page they came from. The line looks like this: ${checkedLine(VENDORS.vercel)}

The sources are screenshots in the repository, one per vendor, taken at a fixed viewport on that date. The filename records the host, the page and the size, for example ${VENDORS.vercel.evidence}. A number the screenshot does not show is not recorded, even when the live page states it. That is why some pages carry fewer competitor numbers than you might expect: I would rather leave a figure out than quote one I cannot show.
...
About ${credits(priceWorkload(WORKLOADS.quietProject).microcredits)} for the quiet project itemized above, most of it one active database hour and stored data. With no requests at all, the only lines left are storage at ${rate("neon.storage.root")} and the script at ${rate("wfp.script")}. Bandwidth from Workers is not charged.
