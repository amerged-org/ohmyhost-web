...
import { DATABASE_PROFILES, PLANS, WORKLOADS } from "../content/sources.js";

export const page: ContentPage = {
  path: "/blog/six-things-that-break-when-a-vibe-coded-app-meets-production",
  title: `Six things that break when a vibe-coded app meets production`,
  description: `What breaks when you deploy a vibe-coded app: localhost URLs, CORS, SQLite, committed secrets, build-time config and mail, and how the agent fixes each.`,
  kind: "article",
  published: "2026-09-20",
  modified: "2026-09-20",
  parent: "/blog",
  crumb: "Six things that break",
  markdown: `# Six things that break when a vibe-coded app meets production

By Sebastian Mertens · September 20, 2026

Deploy a vibe-coded app and the same six things break: localhost URLs, CORS and callback URLs, SQLite, committed secrets, build-time configuration and mail without a verified sender. On ohmyho.st your coding agent fixes each through named MCP tools and deploys from GitHub. The example first month below costs about ${credits(priceWorkload(WORKLOADS.lovableFirstMonth).microcredits)}; the Free plan grants ${number(PLANS.freeCredits)} credits a month.
...
6. Set a monthly ceiling with \`project_budget_set\` if you want one, in continue or stop mode. Every project draws from the organization's one balance; Paid is ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits, and purchased top-ups never expire.
...
${workloadTable(WORKLOADS.lovableFirstMonth)}
...
Not for hosting, Postgres or the platform hostnames: the Free plan grants ${number(PLANS.freeCredits)} credits a month and every project draws from the same balance. A customer domain and managed transactional mail need Paid at ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits, and both keep using credits while they exist.
