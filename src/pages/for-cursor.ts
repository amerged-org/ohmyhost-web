...
export const page: ContentPage = {
  path: "/for/cursor",
  title: `Deploy an app built with Cursor to ohmyho.st`,
  description: `Add one MCP server to Cursor, paste one prompt, and Cursor plans, deploys and verifies your GitHub app on Dev and Prod with Postgres and one credit balance.`,
  kind: "howto",
  modified: "2026-09-20",
  crumb: "Cursor",
  markdown: `# Deploy an app built with Cursor

Add one local MCP server to Cursor, paste one prompt, and Cursor deploys your GitHub repository to Dev and Prod on ohmyho.st with Postgres and a live URL. Free gives ${number(PLANS.freeCredits)} credits a UTC month; Paid is ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits shared by every project. Three static portfolio sites cost about ${credits(priceWorkload(WORKLOADS.threeStaticSites).microcredits)} a month.
...
2. Reload. A running server keeps the tool list it started with, so after an install or upgrade reload the connection in Cursor's MCP settings and confirm that the \`ohmyho\` server lists its tools (62 today).
...
5. Review the plan. \`deployment_plan\` returns the exact commit, the build hold (about ${credits(priceLine("build.sandbox.standard-3", 840))} for 14 reserved minutes, settled to the measured seconds), required secrets and effects. Confirm, and Cursor calls \`deployment_create\`.
...
Each deployed script uses about ${credits(priceLine("wfp.script", 1))} a month while it exists, and every deployment stages one that stays for rollback until cleanup. A customer-owned domain on Prod needs Paid and uses about ${credits(priceLine("domain.custom_hostname", 1))} a month; the domain Skill returns the exact DNS records, or authorizes Cloudflare DNS when you host the zone there. See [environments](https://docs.ohmyho.st/environments) and [domains](https://docs.ohmyho.st/domains).
...
Free: ${number(PLANS.freeCredits)} credits per UTC month. Paid: ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits per period, which also unlocks a customer-owned domain and verified sender mail. Monthly credits expire at period end; purchased top-ups never expire (${number(PLANS.topUpPerUsd)} credits per dollar up to ${usd(100)}, ${number(PLANS.topUpPerUsdAbove100)} per dollar above). Every project draws from the organization's one balance and there is no per-project base fee, which is the point for a portfolio.

${workloadTable(WORKLOADS.threeStaticSites)}

Add a database and the numbers move. One active hour on the Paid standard profile (${DATABASE_PROFILES.standard.cu} CU) is about ${credits(priceLine("neon.compute.scale", DATABASE_PROFILES.standard.cu))}; on the Free profile (${DATABASE_PROFILES.free.cu} CU) about ${credits(priceLine("neon.compute.scale", DATABASE_PROFILES.free.cu))}. Idle compute suspends, but stored data is ${rate("neon.storage.root")} whether the compute is awake or not. A quiet side project with a database costs about ${credits(priceWorkload(WORKLOADS.quietProject).microcredits)} a month. A small app with 100,000 requests, eight active database hours and 2,000 mail recipients costs about ${credits(priceWorkload(WORKLOADS.smallApp).microcredits)}. Requests are ${rate("wfp.requests")}; Workers bandwidth is not charged. Mail needs Paid and a verified sender subdomain, is metered per recipient (To, CC and BCC count separately), and a sender zone uses about ${credits(priceLine("route53.zone", 1))} a month.

Where Vercel wins: Vercel Hobby costs ${usd(VENDORS.vercel.facts.hobby.usd)} a month for one developer seat, and for a static portfolio with no database and no mail it is the cheaper answer. Vercel Pro is ${usd(VENDORS.vercel.facts.pro.usd)} a month before you add a database or a mail provider. ohmyho.st fits once a project needs Postgres, transactional mail or a domain, or once you run several small projects and want one balance instead of one plan each.
...
Yes, when the sites are static or quiet. ${number(PLANS.freeCredits)} credits a UTC month cover three static sites at about ${credits(priceWorkload(WORKLOADS.threeStaticSites).microcredits)}, or one quiet project with a database at about ${credits(priceWorkload(WORKLOADS.quietProject).microcredits)}. A customer-owned domain and sender mail need Paid, and the Free profile runs at ${DATABASE_PROFILES.free.cu} CU.
