...
export const page: ContentPage = {
  path: "/from/lovable",
  title: `Host a Lovable app after export to GitHub — ohmyho.st`,
  description: `Export your Lovable project to GitHub, keep Supabase or import a dump, and let a coding agent deploy the Vite app to ohmyho.st. Login, domain and costs.`,
  kind: "howto",
  modified: "2026-09-20",
  crumb: "From Lovable",
  markdown: `# Host a Lovable app after you export it to GitHub

Export your Lovable project to GitHub, paste one prompt into a coding agent, and ohmyho.st hosts the Vite app from that repository. Keep Supabase or import a dump into managed Postgres. Free gives ${number(PLANS.freeCredits)} credits a month; Paid is ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits, and a small app uses about ${credits(priceWorkload(WORKLOADS.smallApp).microcredits)}.
...
There is no per-project base fee. Every project draws from your organization's one balance: Free gives ${number(PLANS.freeCredits)} credits each UTC month; Paid is ${usd(PLANS.paidUsd)} a month for ${number(PLANS.paidCredits)} credits. Monthly credits expire at the end of the period; a purchased top-up never expires (${number(PLANS.topUpPerUsd)} credits per dollar, ${number(PLANS.topUpPerUsdAbove100)} per dollar above ${usd(100)} in one checkout).

${workloadTable(WORKLOADS.smallApp)}

That workload assumes the database and mail also run here. A linked domain adds the hostname rate, ${rate("domain.custom_hostname")}, about ${credits(priceLine("domain.custom_hostname", 1))} a month, and needs Paid.
...
${workloadTable(WORKLOADS.lovableFrontend)}

Drop the hostname line and the rest fits inside the Free plan's ${number(PLANS.freeCredits)} credits a month, on the \`<three-words>.check.omh.st\` hosts every project gets. Idle costs are honest too: a deployed script keeps using about ${credits(priceLine("wfp.script", 1))} a month and retained database storage ${rate("neon.storage.root")}; Workers bandwidth is not charged. A zero balance starts a seven-day grace period during which everything keeps running. You can put a monthly budget on the project (continue or stop) so a traffic spike cannot surprise you: https://docs.ohmyho.st/budgets.
