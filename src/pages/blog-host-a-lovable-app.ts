...
import { PLANS, SCENARIOS, VENDORS, WORKLOADS } from "../content/sources.js";
import { figureFlow } from "../figures.js";

export const page: ContentPage = {
  path: "/blog/host-a-lovable-app-after-export",
  title: `Hosting a Lovable app after export: the exact path`,
  description: `What Lovable's own docs make you redo by hand after exporting to GitHub, what a coding agent does instead, and what the first month of hosting actually costs.`,
  kind: "article",
  published: "2026-09-20",
  modified: "2026-09-20",
  parent: "/blog",
  crumb: "Host a Lovable app",
  markdown: `# Hosting a Lovable app after export: the exact path
...
Both need Paid, and both are metered rather than bundled. \`domain_paid_plan\` returns the CNAME to set at your registrar and \`domain_paid_apply\` activates the hostname once DNS resolves; a linked hostname costs about ${credits(priceLine("domain.custom_hostname", 1), 0)} a month. Transactional mail needs a delegated sender subdomain that the agent configures with \`mail_domain_set\` and verifies with \`mail_domain_status\`; the zone costs about ${credits(priceLine("route53.zone", 1), 0)} a month and each 1,000 recipients about ${credits(priceLine("ses.{region}.recipients (Essentials)", 1000), 0)}.
...
Both sit inside the ${number(PLANS.paidCredits)} credits that ${usd(PLANS.paidUsd)} a month buys, and both leave room for the extra deployments a move always needs. For comparison, the same app on separate subscriptions starts at ${usd(scenarioUsd(SCENARIOS.threeSubscriptions))} a month before any usage. ${checkedLine(VENDORS.vercel)} ${checkedLine(VENDORS.supabase)} ${checkedLine(VENDORS.resend)}
...
Nothing. Free gives ${number(PLANS.freeCredits)} credits a UTC month with hosting, a database and Dev and Prod hosts, which is enough to deploy the exported app and click through it. A custom domain and mail are what Paid adds.
