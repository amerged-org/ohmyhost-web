...
export function figureBills(competitor: BillsCompetitor): string {
  ...
  const rows = [
    ...
    .join("");
  // The receipts grow with the scenario and the card is fixed, so the summary sits below both and
  // the figure grows with them; the total and the footnote never share a line.
  const receiptsBottom = 36 + scenario.parts.length * 74 - 16;
  const summary = Math.max(receiptsBottom, 252) + 32;
  const note = summary + 22;
  const height = note + 22;
  const svg = `<svg viewBox="0 0 960 ${height}" role="img" aria-label="${scenario.parts.length} subscriptions at ${usd(total)} a month versus one ohmyho.st balance at ${usd(10)} a month for every project" xmlns="http://www.w3.org/2000/svg"><defs><radialGradient id="bills-glow" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="currentColor" stop-opacity=".08"/><stop offset="100%" stop-color="currentColor" stop-opacity="0"/></radialGradient></defs>${receipts}${value(total, `${usd(total)} /mo`, 420, summary)}<text x="40" y="${note}" ${MONO} font-size="11" fill="var(--subtle)">${scenario.parts.length} accounts · 1 project · ${extraProject}</text><line x1="480" y1="36" x2="480" y2="${note + 6}" stroke="var(--border)"/><ellipse cx="730" cy="160" rx="230" ry="120" fill="url(#bills-glow)"/><rect x="540" y="36" width="380" height="216" rx="12" fill="none" stroke="var(--foreground)"/><path d="${OMEGA}" transform="translate(556 44) scale(.32)" fill="none" stroke="var(--foreground)" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"/><text x="600" y="66" ${SANS} font-size="15" font-weight="600" fill="var(--foreground)">ohmyho.st · every project</text>${rows}${value(10, `${usd(10)} /mo`, 920, summary)}<text x="540" y="${note}" ${MONO} font-size="11" fill="var(--subtle)">1 balance · ${number(1000)} credits a month</text></svg>`;
  return figure(
