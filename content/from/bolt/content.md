...
**Mail.** If your Bolt app sends its own mail (welcome, reset, receipts) through Supabase Auth or an external provider, that keeps working unchanged. If you want ohmyho.st to send, the agent runs `mail_domain_set` for a sender subdomain, you add four NS records, and `mail_domain_status` reports DKIM and SPF verification. The sender zone is metered at {{ rate route53.zone }}, about {{ credits unit.mailSenderZoneMonth }} a month, and every send at {{ rate ses.{region}.recipients (Essentials) }}; To, CC and BCC each count as one recipient. Transactional mail is sent from the platform mail region regardless of your project's region. Read [email](https://docs.ohmyho.st/email).
