...
A project budget. `project_budget_set` in `stop` mode rejects new billable work with `project_budget_exceeded` once the UTC-month threshold is reached and pauses the project's traffic. Automatic recharge stays off unless the Owner turns it on, so nothing buys more credits on its own; a zero balance starts a seven-day grace period during which funded services keep running.
