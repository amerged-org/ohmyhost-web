...
Annex IV to the ohmyho.st DPA. Version: September 24, 2026.
...
For customer workload data, amerged generally acts as processor and the infrastructure recipient as subprocessor: Module Three applies where SCCs are the required mechanism. For account or business data for which amerged is controller and a non-EEA recipient is processor, Module Two applies where appropriate. The applicable provider agreement supplies the contracting importer, selected options and completed annexes for that engagement. Cloudflare and WorkOS publish contractual SCC arrangements in their data-processing terms; Neon’s [published DPA](https://neon.com/pdf/DPA.pdf) also provides transfer clauses, whose applicability and module depend on the signed account agreement. Resend’s [published DPA](https://resend.com/legal/dpa) states that transfers from the EEA and Switzerland are made under the EU SCCs and transfers from the UK under the UK SCCs; its applicability likewise follows the account terms. Resend also states that it participates in the EU–US Data Privacy Framework; under the rule above, that is relied on only once its current certification has been checked. These provider instruments govern the onward transfer rather than incorrectly identifying an EU customer as a direct exporter to every infrastructure supplier.
